(() => {
  ...
  function renderPool(){
    ...
    const intelFilter=val('hsIntelFilter');
    let rows=S.pool.filter(r=>{
      const p=r.scouting_players||{};
      const intel=S.intel.find(i=>i.scouting_player_id===r.scouting_player_id);
      const matchesText=!q||[p.gamertag,p.primary_position,p.platform,r.status,r.projected_role,intel?.role_chip,intel?.player_name].some(v=>String(v||'').toLowerCase().includes(q));
      const matchesPos=!pos||String(p.primary_position||'').toUpperCase()===pos;
      const matchesStatus=!status||r.status===status;
      const hasIntel=!!intel;
      const matchesIntel=!intelFilter||(intelFilter==='yes'&&hasIntel)||(intelFilter==='no'&&!hasIntel);
    ...
    $('hsPoolBody').innerHTML=shown.map(r=>{
      const p=r.scouting_players||{};
      const intel=S.intel.find(i=>i.scouting_player_id===r.scouting_player_id);
      const statusNow=r.status||'unscouted';
      const quick=(value,label)=>`<button type="button" class="hs-quick-btn ${statusNow===value?'active':''}" data-hs-quick="${value}" data-hs-pool-id="${r.id}">${label}</button>`;
      return `<tr data-hs-player="${r.id}">
        <td>
          <b>${esc(p.gamertag||'Unknown')}</b><br>
          <small>${esc(p.platform||'')}</small>
          <div class="hs-quick-actions">
            ${quick('bid_target','Target')}
            ${quick('watch','Watch')}
            ${quick('pass','Pass')}
          </div>
        </td>
        <td>${esc(p.primary_position||'—')}</td>
        <td><span class="hs-tag">${esc(statusNow.replaceAll('_',' '))}</span></td>
        <td class="hs-price-cell">${intel?mval(intel.fair_value_m):'—'}</td>
        <td class="hs-price-cell">${intel?mval(intel.likely_price_m):'—'}</td>
        <td class="hs-price-cell">${intel?mval(intel.walk_above_m):'—'}</td>
        <td>${money(r.target_bid)}</td>
        <td>${money(r.max_bid)}</td>
      </tr>`;
    }).join('');
    ...
    document.querySelectorAll('[data-hs-quick]').forEach(btn=>{
      btn.onclick=e=>{
        e.preventDefault();
        e.stopPropagation();
        quickStatus(btn.dataset.hsPoolId,btn.dataset.hsQuick,btn);
      };
    });
  }

  async function quickStatus(poolId,status,button){
    const r=S.pool.find(x=>x.id===poolId);
    if(!r||!['bid_target','watch','pass'].includes(status))return;
    const old=button?.textContent||'';
    if(button){button.disabled=true;button.textContent='…';}
    const payload={status,updated_by:state().user.id,updated_at:new Date().toISOString()};
    const q=await db().from('team_scouting_pool').update(payload).eq('id',poolId);
    if(q.error){
      if(button){button.disabled=false;button.textContent=old;}
      msg('hsStatus',q.error.message);
      return;
    }
    r.status=status;
