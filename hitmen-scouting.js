(() => {
  ...
  function renderIntel(pid){
    ...
    const fold=x.raw_payload?.fold||{};
    box.innerHTML=`<div class="hs-report">${fold.call?`<p style="font-size:1.05rem"><b>ChelScout read:</b> ${esc(fold.call)}</p>`:''}${fold.why?`<p><b>Why:</b> ${esc(fold.why)}</p>`:''}<div class="hs-grades"><div class="hs-grade"><small>Role</small><b>${esc(x.role_chip||x.role_band||'—')}</b></div><div class="hs-grade"><small>Fair Value</small><b>${mval(x.fair_value_m)}</b></div><div class="hs-grade"><small>Likely Bid</small><b>${mval(x.likely_price_m)}</b></div><div class="hs-grade"><small>Walk Above</small><b>${mval(x.walk_above_m)}</b></div></div>
