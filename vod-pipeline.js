(() => {
  ...
  function install(){
    ...
    panel.innerHTML=`<div class="eyebrow">ONE-UPLOAD PIPELINE</div><h3 style="margin:6px 0 8px">Upload Once · Analyze By Period</h3><p>Select the actual recording once. Wildman sends that file to the Railway worker, processes only the period windows you already built, and keeps the same upload for retries.</p><input id="vodPipelineFile" class="field" type="file" accept="video/mp4,video/quicktime,.mp4,.mov"><div class="vod-actions" style="margin-top:10px"><button id="vodStartPipeline" class="small-btn primary" type="button">Upload & Start Pipeline</button><button id="vodCheckPipeline" class="small-btn" type="button">Check Status</button><button id="vodRetryPipeline" class="small-btn" type="button">Retry AI</button></div><small id="vodPipelineStatus">Build the real P1/P2/P3 boundaries first, then choose the recording once.</small>`;
  ...
  async function start(){
    ...
    try{
      ...
      if(periods.length<3)throw new Error('Build Period 1, Period 2 and Period 3 boundaries first. The recording only needs to be selected once after that.');
      ...
      const job=await workerFetch('/jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({game_id:review.id,title:review.title||'Game VOD',vod_url:review.vod_url||'',players:'',periods:periods.map(p=>({label:p.label,start:p.start,end:p.end}))})});
      ...
      setStatus('Upload complete. Railway is processing the saved period windows.','good');
  ...
  async function retry(){
    try{
      const review=await currentReview(); if(!review?.worker_job_id)throw new Error('There is no saved worker job to retry.');
  ...
  async function checkJob(jobId,reviewId,loud=false){
    try{
      ...
      const done=['ready_for_review','failed','expired','awaiting_ai'].includes(job.status);
      if(job.status==='awaiting_ai')setStatus('Video validated and split into period-sized work. AI is not connected to Railway yet. No re-upload is needed once the AI connection is added.','warn');
